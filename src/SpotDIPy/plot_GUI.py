...
class BaseCanvas(FigureCanvasQTAgg):
    def __init__(self, incl=90, kind='multi'):
        self.fig = Figure()
        ...
        if kind == 'multi':
            ...
            self.fig.axes[4].set_visible(False)
...
class PlotGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, DIP, plot_params):
        ...
        self.spherical_dplot = BaseCanvas(incl=DIP.params['incl'], kind='multi')
        self.spherical_dtoolbar = NavigationToolbar2QT(self.spherical_dplot, self)

        self.spherical_cplot = BaseCanvas(kind='single')
        self.spherical_ctoolbar = NavigationToolbar2QT(self.spherical_cplot, self)
        self.spherical_cplot.hide()
        self.spherical_ctoolbar.hide()

        spherical_plot_dphase_button = QtWidgets.QPushButton('Plot Default Phases')
        spherical_plot_cphase_button = QtWidgets.QPushButton('Plot Specific Phase -->')
        spherical_plot_cphase_lineedit = QtWidgets.QLineEdit('0.0')
        spherical_plot_cphase_lineedit.setMaximumWidth(100)
        spherical_plot_hlayout = QtWidgets.QHBoxLayout()
        spacerItem = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum)
        spherical_plot_hlayout.addItem(spacerItem)
        spherical_plot_hlayout.addWidget(spherical_plot_dphase_button)
        spherical_plot_hlayout.addWidget(spherical_plot_cphase_button)
        spherical_plot_hlayout.addWidget(spherical_plot_cphase_lineedit)
        spacerItem = QtWidgets.QSpacerItem(40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum)
        spherical_plot_hlayout.addItem(spacerItem)

        spherical_gbox_glayout.addWidget(self.spherical_dtoolbar, 0, 0)
        spherical_gbox_glayout.addWidget(self.spherical_dplot, 1, 0)
        spherical_gbox_glayout.addWidget(self.spherical_ctoolbar, 2, 0)
        spherical_gbox_glayout.addWidget(self.spherical_cplot, 3, 0)
        spherical_gbox_glayout.addLayout(spherical_plot_hlayout, 4, 0)
        ...
        self.plot_mercator(crmap=self.mapprojs['rmap_int'], cextent=np.rad2deg(self.mapprojs['extent']))
        self.plot_mollweide(crmap=self.mapprojs['rmap_int'], xlats=self.mapprojs['xlats'], xlongs=self.mapprojs['xlongs'])
        self.plot_spherical(crmap=self.mapprojs['rmap_int'], mlats=self.mapprojs['mlats'], mlongs=self.mapprojs['mlongs'])
        self.plot_3d_surface()

        spherical_plot_dphase_button.clicked.connect(lambda: self.plot_spherical(crmap=self.mapprojs['rmap_int'],
                                                                                 mlats=self.mapprojs['mlats'],
                                                                                 mlongs=self.mapprojs['mlongs']))

        spherical_plot_cphase_button.clicked.connect(lambda: self.plot_spherical(crmap=self.mapprojs['rmap_int'],
                                                                                 mlats=self.mapprojs['mlats'],
                                                                                 mlongs=self.mapprojs['mlongs'],
                                                                                 phase=float(spherical_plot_cphase_lineedit.text())))
    ...
    def plot_mercator(self, crmap, cextent):
        ...
        for i, mode in enumerate(self.DIP.conf):
            if mode in ['line', 'lc']:
                ...
                ax.plot([obslong, obslong], ranges[i], '-', color=colors[i], linewidth=1)
                if 0.0 in phases:
                    ax.plot([0, 0], ranges[i], '-', color=colors[i], linewidth=1)
    ...
    def plot_mollweide(self, crmap, xlongs, xlats):
        ...
        for i, mode in enumerate(self.DIP.conf):
            if mode in ['line', 'lc']:
                ...
                ax.plot(np.deg2rad([obslong, obslong]), np.deg2rad(ranges[i]), '-', color=colors[i], linewidth=1)
                if 0.0 in phases:
    ...
    def plot_spherical(self, crmap, mlats, mlongs, phase=None):
        ...
        if phase is not None:
            self.spherical_dplot.hide()
            self.spherical_dtoolbar.hide()
            self.spherical_cplot.show()
            self.spherical_ctoolbar.show()

            fig = self.spherical_cplot.fig
            ax = self.spherical_cplot.fig.axes[0]

            ax.cla()

            m = Basemap(ax=ax, projection='ortho', lat_0=90 - self.DIP.params['incl'], lon_0=360 - 360 * phase)
            ...
            ax.set_title('Phase = ' + str('%0.2f' % phase), pad=10)

            try:
                self.sph_cbar.remove()
            except:
                pass

            divider = make_axes_locatable(ax)
            cax = divider.append_axes("right", size="5%", pad=0.5)
            self.sph_cbar = plt.colorbar(ScalarMappable(norm=img.norm, cmap=img.cmap), cax=cax,
                                         orientation='vertical',
                                         format="%0.4f", ticks=np.linspace(self.vmin, self.vmax, 6))
            self.sph_cbar.ax.tick_params(labelsize=self.tls)
            self.sph_cbar.set_label(r'$\frac{I}{I_{phot}}$', fontsize=self.fs + 5)

            fig.tight_layout()

            self.spherical_cplot.draw()

        else:
            self.spherical_cplot.hide()
            self.spherical_ctoolbar.hide()
            self.spherical_dplot.show()
            self.spherical_dtoolbar.show()

            fig = self.spherical_dplot.fig
            axs = self.spherical_dplot.fig.axes

            ms = [self.spherical_dplot.m1, self.spherical_dplot.m2,
                  self.spherical_dplot.m3, self.spherical_dplot.m4]
            ...
            divider = make_axes_locatable(self.spherical_dplot.fig.axes[-1])
            cax = divider.append_axes("right", size="75%", pad=0.05)
            self.sph_cbar = plt.colorbar(ScalarMappable(norm=img.norm, cmap=img.cmap), cax=cax,
                                         orientation='vertical',
                                         format="%0.4f", ticks=np.linspace(self.vmin, self.vmax, 6))
            self.sph_cbar.ax.tick_params(labelsize=self.tls)
            ...
            SubplotToolQt(fig.canvas.figure, fig.canvas.parent())
