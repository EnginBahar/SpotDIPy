        ...
        if mode != 'lc':
            ...
            for i, itime in enumerate(times):
                ...
                if conf[mode]['scaling']['method'] == 'mean':
                    ...
                    owavs = ((ovels * np.average(conf[mode]['wrange'])) / ac.c.to(au.kilometer / au.second).value
                             + np.average(conf[mode]['wrange']))
...
def ldcs_rgi_prep(temps, loggs, law, mh, model, data_path, mu_min, wrange=None, passband=None, cpu_num=1):

    input_args = []
    for temp in temps:
        for logg in loggs:
            input_args += [(temp, logg, law, mh, model, data_path, mu_min, wrange, passband)]
    ...
    # points = (temps, loggs)
    # if law == 'linear':
    #     rgi = [RegularGridInterpolator(points, parts[:, 0].reshape(len(temps), len(loggs)), bounds_error=False),
    #            RegularGridInterpolator(points, np.zeros((len(temps), len(loggs))), bounds_error=False),
    #            RegularGridInterpolator(points, parts[:, 1].reshape(len(temps), len(loggs)), bounds_error=False)]
    #
    # else:
    #     rgi = [RegularGridInterpolator(points, parts[:, 0].reshape(len(temps), len(loggs)), bounds_error=False),
    #            RegularGridInterpolator(points, parts[:, 1].reshape(len(temps), len(loggs)), bounds_error=False),
    #            RegularGridInterpolator(points, parts[:, 2].reshape(len(temps), len(loggs)), bounds_error=False)]

    if law == 'linear':
        rgi = [RectBivariateSpline(temps, loggs, parts[:, 0].reshape(len(temps), len(loggs))),
               RectBivariateSpline(temps, loggs, np.zeros((len(temps), len(loggs)))),
               RectBivariateSpline(temps, loggs, parts[:, 1].reshape(len(temps), len(loggs)))]

    else:
        rgi = [RectBivariateSpline(temps, loggs, parts[:, 0].reshape(len(temps), len(loggs))),
               RectBivariateSpline(temps, loggs, parts[:, 1].reshape(len(temps), len(loggs))),
               RectBivariateSpline(temps, loggs, parts[:, 2].reshape(len(temps), len(loggs)))]
...
def ldcs_rgi_prep_main(args):
    ...
    temp, logg, law, mh, model, data_path, mu_min, wrange, passband = args
...
def _integrate_I_mu(sc, wavelength_range, mode, custom_wavelengths=None, custom_throughput=None):
    ...
    """ Sourced from the exotic_ld Python package for the calculation of Imu_0 """
...
def test_lc_plot(DIP, plotp):
    ...
    if 'lc' in DIP.conf:
        ...
        ax2.plot(epochs, fluxs - recons_slc, 'ko', ms=plotp['markersize'])
...
def test_map_plot(DIP, fake_fssc, fake_fssh, fake_total_fs, recons_total_fs, plotp):
    ...
    #     _, _, frmap, fextent, _, _ = p2m_to_rect_map(ints=fake_fss, grid_xyzs=DIP.surface_grid['grid_xyzs'], subno=2)
    #     _, _, crmap, cextent, _, _ = p2m_to_rect_map(ints=recons_fss, grid_xyzs=DIP.surface_grid['grid_xyzs'], subno=2)
    #
    # if surface_grid['method'] == 'trapezoid':
    #     nlons = surface_grid['nlons'].copy()
    #
    #     _, _, frmap, fextent, _, _ = td_to_rect_map(nlons=nlons, ints=fake_fss)
    #     _, _, crmap, cextent, _, _ = td_to_rect_map(nlons=nlons, ints=recons_fss)
    #
    # if surface_grid['method'] == 'healpy':
    #     _, _, frmap, fextent, _, _ = hp_to_rect_map(ints=fake_fss, nside=surface_grid['nside'], xsize=1000, ysize=500)
    #     _, _, crmap, cextent, _, _ = hp_to_rect_map(ints=recons_fss, nside=surface_grid['nside'], xsize=1000, ysize=500)
    ...
    # clb2.set_label(r'$\frac{I}{I_{phot}}$', fontsize=12)
    ...
    # clb3.set_label(r'$\frac{I}{I_{phot}}$', fontsize=12)
