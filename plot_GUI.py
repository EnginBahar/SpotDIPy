...
class PlotGUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, DIP, plot_params):
        ...
        self.lmbds = DIP.opt_results['lmbds']
        self.chisqs = DIP.opt_results['total_chisqs']
        self.mems = DIP.opt_results['mems']
        self.maxcurve = DIP.opt_results['maxcurve']
        ...
        profile_tab.setTabVisible(3, False)
    ...
    def plot_line_profiles(self):
        ...
        for i, time in enumerate(spl_line_slps):
            ...
            if seb:
                ...
                ax1.plot(self.DIP.idc['line']['vels'], obs_line_prf + i * line_sp, 'ko', ms=self.ms)
                ax2.plot(self.DIP.idc['line']['vels'], residual + i * line_sr, 'ko', ms=self.ms)
    ...
    def plot_lc_profile(self):
        ...
        if 'lc' in self.DIP.conf:
            ...
            if seb:
                ...
                ax2.errorbar(epochs, fluxs - recons_slc, yerr=errs, fmt='o', color='k', ms=self.ms)
            else:
                ax1.plot(epochs, fluxs, 'ko', label='Observed Light Curve', ms=self.ms)
                ax2.plot(epochs, fluxs - recons_slc, 'ko', ms=self.ms)

            ax1.plot(epochs, recons_slc, 'r', label='Spotted Model', linewidth=self.lw)
    ...
    def plot_mol_profiles(self, sep_prf_ledit, sep_res_ledit, err_cbox, prf_plot, mol_obs_data,
        ...
        # ax1.plot([], [], 'ko', label='Observed Molecular Profiles', ms=self.ms)
    ...
    def plot_mercator(self, crmap, cextent):
        ...
        # obslong = 360 * (1.0 - com_phases)
        # ax.plot([obslong, obslong], [-85, -75], 'b',  linewidth=1)
        # if 0.0 in com_phases:
    ...
    def plot_mollweide(self, crmap, xlongs, xlats):
        ...
        # obslong = 360 * (1.0 - com_phases) - 180
    ...
    def plot_3d_surface(self):
        ...
        # point_data = tmesh.mlab_source.dataset.point_data
        # point_data.scalars = tdmap
        # point_data.scalars.name = 'Point data'
        # point_data.update()
        #
        # mesh2 = self.trid_plot_vis.scene.mlab.pipeline.set_active_attribute(tmesh, point_scalars='Point data')
        # self.trid_plot_vis.scene.mlab.pipeline.surface(mesh2)
    ...
    def plot_lamda_search(self):
        ...
        best_lmbd = self.lmbds[self.maxcurve]

        ax1.plot(self.chisqs, self.mems, 'ko', ms=self.ms)
        ax1.plot(self.chisqs[self.maxcurve], self.mems[self.maxcurve], 'ro', ms=self.ms + 1,
